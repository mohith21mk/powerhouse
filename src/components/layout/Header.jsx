...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-30 bg-[#0B0F17]/95 backdrop-blur-md border-b border-[#1E293B] px-4 sm:px-8 py-3.5">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        {/* Left Welcome Area */}
        <div className="flex items-center gap-3">
          <button
            onClick={onOpenMobileMenu}
            className="p-2 -ml-2 rounded-xl text-slate-400 hover:text-white hover:bg-[#141C2B] lg:hidden cursor-pointer"
        ...
        <div className="flex items-center gap-3 self-end lg:self-center flex-wrap">
          ...
          <div className="relative hidden md:block">
            ...
            <input
              ...
              className="w-72 bg-[#111827] border border-[#1E293B] text-slate-200 placeholder:text-slate-400 text-xs rounded-xl pl-9 pr-4 py-2 focus:outline-none focus:border-blue-500 transition-colors"
            />
          </div>
          ...
          <div className="relative" ref={notifRef}>
            <button
              ...
              className="relative p-2 rounded-xl bg-[#111827] border border-[#1E293B] text-slate-300 hover:text-white hover:bg-[#141C2B] transition-colors cursor-pointer"
              aria-label="Notifications"
            >
              <Bell className="w-4 h-4" />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[17px] h-[17px] px-1 bg-rose-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center ring-2 ring-[#0B0F17]">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 sm:w-88 bg-[#111827] rounded-2xl shadow-2xl border border-[#1E293B] py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="px-4 py-2.5 border-b border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-xs text-white">Notifications</span>
                    <span className="px-1.5 py-0.5 text-[10px] rounded-full bg-blue-950 text-blue-400 font-bold border border-blue-800">
                      {unreadCount} new
                    </span>
                  </div>
                  {unreadCount > 0 && (
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-slate-800/60">
                  {notifications.map((item) => (
                    <div
                      ...
                      className={`p-3.5 hover:bg-[#141C2B] transition-colors cursor-pointer flex items-start gap-3 ${
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[#111827] border border-[#1E293B] text-slate-300 hover:text-white hover:bg-[#141C2B] transition-colors cursor-pointer"
          ...
          <div className="relative" ref={businessRef}>
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#111827] border border-[#1E293B] hover:bg-[#141C2B] text-slate-200 hover:text-white text-xs font-semibold transition-colors cursor-pointer"
            ...
            {showBusinessMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-[#111827] rounded-2xl shadow-2xl border border-[#1E293B] py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="px-3.5 py-2 border-b border-slate-800">
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    Current Active Unit
                  </p>
                  <p className="text-xs font-bold text-white mt-1">{businessName}</p>
                </div>
                <div className="p-2 space-y-1">
                  {auth?.userBusinesses && auth.userBusinesses.length > 1 && (
                    <div className="mb-2 pb-2 border-b border-slate-800">
                      <p className="text-[10px] font-bold text-slate-500 uppercase px-2 mb-1">Your Businesses</p>
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-xs text-slate-200 hover:bg-[#141C2B] rounded-xl flex items-center gap-2 cursor-pointer font-medium"
                  >
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className="flex items-center gap-2.5 p-1 pl-1.5 rounded-xl hover:bg-[#141C2B] border border-[#1E293B] transition-colors cursor-pointer group"
