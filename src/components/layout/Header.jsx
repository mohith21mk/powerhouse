...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-30 bg-[#0B0F17]/95 backdrop-blur-md border-b border-[#1E293B] px-3 sm:px-4 h-11 min-h-[44px] flex items-center justify-between gap-2.5 shrink-0">
      {/* Left Welcome Area */}
      <div className="flex items-center gap-2 min-w-0">
        <button
          onClick={onOpenMobileMenu}
          className="p-1 -ml-1 rounded text-slate-400 hover:text-white hover:bg-[#141C2B] lg:hidden cursor-pointer shrink-0"
        ...
        <div className="min-w-0">
          <h1 className="text-[16px] sm:text-[18px] font-bold text-white tracking-tight flex items-center gap-1.5 truncate leading-none">
            Good morning, {firstName ? firstName.toUpperCase() : 'USER'}! <span className="text-sm">👋</span>
          </h1>
          <p className="text-[9.5px] text-slate-400 truncate leading-none mt-0.5">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="relative hidden md:block">
          ...
          <input
            ...
            className="w-40 lg:w-48 h-7 bg-[#111827] border border-[#1E293B] text-slate-200 placeholder:text-slate-400 text-[10.5px] rounded-md pl-6 pr-2 py-0.5 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="relative w-7 h-7 rounded-md bg-[#111827] border border-[#1E293B] text-slate-300 hover:text-white hover:bg-[#141C2B] transition-colors cursor-pointer flex items-center justify-center"
            aria-label="Notifications"
          >
            <Bell className="w-3 h-3" />
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[13px] h-[13px] px-0.5 bg-rose-600 text-white text-[8px] font-bold rounded-full flex items-center justify-center ring-2 ring-[#0B0F17]">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-1.5 w-72 sm:w-80 bg-[#111827] rounded-xl shadow-2xl border border-[#1E293B] py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              <div className="px-3 py-2 border-b border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className="font-semibold text-[11px] text-white">Notifications</span>
                  <span className="px-1.5 py-0.2 text-[9px] rounded-full bg-blue-950 text-blue-400 font-bold border border-blue-800">
                    {unreadCount} new
                  </span>
                </div>
                {unreadCount > 0 && (
        ...
        <button
          ...
          className="w-7 h-7 rounded-md bg-[#111827] border border-[#1E293B] text-slate-300 hover:text-white hover:bg-[#141C2B] transition-colors cursor-pointer flex items-center justify-center"
        ...
        <div className="relative" ref={businessRef}>
          <button
            ...
            className="h-7 flex items-center gap-1.5 px-2 rounded-md bg-[#111827] border border-[#1E293B] hover:bg-[#141C2B] text-slate-200 hover:text-white text-[10.5px] font-medium transition-colors cursor-pointer"
            aria-label="Select Business"
            title={businessName}
          >
            <Building className="w-3 h-3 text-blue-400 shrink-0" />
            <span className="max-w-[130px] truncate text-[10.5px]">{businessName}</span>
            <ChevronDown className="w-2.5 h-2.5 text-slate-400 shrink-0" />
          </button>

          {showBusinessMenu && (
            <div className="absolute right-0 mt-1.5 w-60 bg-[#111827] rounded-xl shadow-2xl border border-[#1E293B] py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              <div className="px-3 py-1.5 border-b border-slate-800">
                <p className="text-[9.5px] font-bold text-slate-400 uppercase tracking-wider">
                  Current Active Unit
                </p>
                <p className="text-[11px] font-bold text-white mt-0.5">{businessName}</p>
              </div>
              <div className="p-1.5 space-y-0.5">
                {auth?.userBusinesses && auth.userBusinesses.length > 1 && (
                  <div className="mb-1.5 pb-1.5 border-b border-slate-800">
                    <p className="text-[9px] font-bold text-slate-500 uppercase px-1.5 mb-0.5">Your Businesses</p>
                    {auth.userBusinesses.map((b) => (
                      <button
                        ...
                        className={`w-full px-2 py-1 text-left text-[11px] rounded flex items-center justify-between cursor-pointer ${
                      ...
                      >
                        ...
                        <span className="text-[9px] text-slate-500">{b.city}</span>
                      </button>
                    ))}
                  </div>
                )}
